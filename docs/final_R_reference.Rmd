...
**dnorm() function - gives the probability of observing a specified value from a normally distributed population.**
```{r}
# Finding the probability of observing the value 1.2 from a normally distributed population with a mean = 2 and a standard deviation = 2
dnorm(1.2, mean = 2, sd = 2)
```

**pnorm() function - gives the probability of observing on or below a specified value from a normally distributed population.**
```{r}
# Finding the probability of observing a value less than 1.2 from a normally distributed population with a mean = 2 and a standard deviation = 2
pnorm(1.2, mean = 2, sd = 2)
```

**qnorm() function - gives the value that is the specified percentile from a normally distributed population.**
```{r}
# Finding the value that is the 85th percentile of a normal distribution with a mean = 70 and standard deviation = 3
qnorm(0.85, mean = 70, sd = 3)
```

**dbinom() function - gives the probability of getting a specific number of successes based on the given distribution parameters.**
```{r}
# Finding the probability of observing a count of exactly 3 successes in a binomial distribution with parameters n = 4 and p = 0.75
dbinom(3, size = 4, prob = 0.75)
```

**pbinom() function - gives the probability of getting on or below a specific value based on the given distribution parameters.**
```{r}
# Finding the probability of observing a count of exactly 3 successes in a binomial distribution with parameters n = 4 and p = 0.75.
pbinom(3, size = 4, prob = 0.75)
```

**qbinom() function - gives the specified quantile of a binomial distribution based on the given distribution parameters.**
```{r}
# Finding the 10th quantile of a binomial distribution with trials = 10 and p = 0.4
